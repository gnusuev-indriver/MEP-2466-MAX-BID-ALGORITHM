...
def plot_conversions_by_time(df_grouped, grouped_column, plot_root_path, is_before=True, is_show=True):
    ...
    fig = make_subplots(rows=4, cols=1, 
                        row_titles=['absolutes', 'main conversions', 'bid conversions', 'Diffs'])
    ...
    fig.add_trace(go.Scatter(x=df_grouped[df_grouped.group_name == 'Control'][grouped_column], 
                             y=df_grouped[df_grouped.group_name == 'A']['bid2accept'].reset_index(drop=True) - df_grouped[df_grouped.group_name == 'Control']['bid2accept'].reset_index(drop=True),
                             mode='lines',
                             name='bid2accept (Diff AvsControl)',
...
# def plot_algo_heatmap(df, min_samples=10, is_before=False):
#     """
...
#             raise ValueError(f"Колонка {col} отсутствует в df")
...
#         for geo in df_group['city_id'].unique():
#             print(geo)
#             for type_name in df_group['order_type'].unique():
#                     print(type_name)
#                     # Фильтруем данные
#                     df_geo_type = df_group[(df_group['city_id'] == geo) & 
#                                         (df_group['order_type'] == type_name)]
...
#                     # Подсчитываем долю записей с algo_name_new == 'algo_bidmph' для каждой комбинации eta_bin и duration_bin
#                     algo_counts = df_geo_type[df_geo_type['algo_name_new'] == 'algo_bidmph'].groupby(['eta_bin', 'duration_bin']).size().reset_index(name='algo_count')
                    
#                     # Объединяем данные
#                     merged = pd.merge(bin_counts, algo_counts, on=['eta_bin', 'duration_bin'], how='left')
#                     merged['algo_count'] = merged['algo_count'].fillna(0)
#                     merged['algo_bidmph_share'] = merged['algo_count'] / merged['count']
...
#                     pivot_table = filtered_bins.pivot(index='duration_bin', columns='eta_bin', values='algo_bidmph_share')
...
#                     # Настраиваем макет
#                     fig.update_layout(
#                         title_text=f"Share of algo_bidmph: {group}",
#                         width=900,
#                         height=700,
