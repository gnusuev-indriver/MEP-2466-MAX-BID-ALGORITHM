...
alpha_values AS (
    SELECT alpha_value
    FROM UNNEST([0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4]) AS alpha_value
),
...
incity_details_raw AS (
    ...
    WHERE TRUE
        AND created_date_order_part >= DATE_SUB(DATE('2025-01-29'), INTERVAL 1 DAY)
        AND created_date_order_part <= DATE_ADD(DATE('2025-02-26'), INTERVAL 1 DAY)
        AND DATE(TIMESTAMP(FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:%S', order_timestamp), timezone)) BETWEEN DATE('2025-01-29') AND DATE('2025-02-26')
        AND city_id = 4180
),
...
order_global_info AS (
    ...
    WHERE uuid IN (SELECT DISTINCT order_uuid FROM incity_details_base)
      AND DATE(created_at) >= DATE('2025-01-29') 
      AND DATE(created_at) <= DATE('2025-02-26')
    GROUP BY uuid
),
...
bids_raw AS (
    ...
    WHERE status = 'BID_STATUS_ACTIVE'
      ...
      AND DATE(created_at) BETWEEN DATE('2025-01-29') AND DATE('2025-02-26')
),
...
evaluated_bids AS (
    SELECT
        ...
        (SAFE_DIVIDE(
                                SAFE_DIVIDE(bid_price_value, (t1_duration_seconds + avg_rta_seconds_t) + GREATEST(0, (t2_eta_seconds - avg_rta_seconds_t))),
                                SAFE_DIVIDE(GREATEST(price_highrate_value, price_start_value), (t1_duration_seconds + avg_rta_seconds_t))
                              )  > 1 + av.alpha_value) AS is_bad_bid_2
...
overall_bad_bid_share AS (
    SELECT
        ...
        SUM(CAST(is_bad_bid_2 AS INT64)) AS count_bad_bids_2,
        SAFE_DIVIDE(SUM(CAST(is_bad_bid AS INT64)), COUNTIF(max_permissible_bid IS NOT NULL)) AS share_of_bad_bids,
        SAFE_DIVIDE(SUM(CAST(is_bad_bid_2 AS INT64)), COUNTIF(max_permissible_bid IS NOT NULL)) AS share_of_bad_bids_2
...
accepted_tender_evaluation AS (
    SELECT
        ...
        bws.is_bad_bid_2 AS is_accepted_tender_bad_2,
...
done_orders_with_bad_tender_counts AS (
    SELECT
        ...
        COUNTIF(is_accepted_tender_bad_2 = TRUE AND is_accepted_tender_rule_applicable = TRUE) AS num_done_orders_with_bad_tender_2
...
final_metric_done_orders_bad_tender AS (
    SELECT
        ...
        COALESCE(dowbtc.num_done_orders_with_bad_tender_2, 0) AS num_done_orders_with_bad_tender_2,
        SAFE_DIVIDE(
        ...
        SAFE_DIVIDE(
            COALESCE(dowbtc.num_done_orders_with_bad_tender_2, 0),
            acoc.count_all_completed_orders
        ) AS share_done_orders_with_bad_tender_2 -- Denominator is ALL completed orders
...
SELECT
    ...
    m1.count_bad_bids                           AS abdbids_count,
    m1.share_of_bad_bids                        AS abdbids_share,
    m1.share_of_bad_bids_2 AS abdbids_share_2,
    ...
    m2.share_done_orders_with_bad_tender_2        AS rides_badbids_share_2,
